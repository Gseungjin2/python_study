...
'''
도전과제2-빈칸을 맞춘 글자로 바꾸기

placeholder = '_' * len(chosen_word) 
파이썬에서 * 연산자는 문자열을 반복하는 데 사용할 수 있어요.
즉, 어떤 문자열을 몇 번 반복할지 숫자를 곱해주면 그만큼 반복된 문자열이 만들어집니다.
정리:
len(word): 단어의 글자 수를 알려줌.
'_' * len(word): 그 글자 수만큼 밑줄을 반복해서 만듦.
결과적으로 placeholder는 단어 길이와 똑같은 수의 밑줄로 채워진 문자열이 되는 거예요.
